...
func probability(procs []*score.Process, coeffs []float64, time float64,
	matchMoments func(m, v float64) (x, y, z float64)) float64 {
	mTot, vTot := 0.0, 0.0
	for i, proc := range procs {
		c := coeffs[i]
		m, v := proc.Predict(time)
		mTot += c * m
		vTot += c * c * v
	}
	logpart, _, _ := matchMoments(mTot, vTot)
